...
app.post('/api/razorpay/webhook', (req, res) => {
  ...
  console.log(`[Razorpay Webhook] Event: ${event} | SubID: ${subscriptionId} | Tier: ${tier} | Status: ${status}`);
...
function generateFallbackColors(prompt: string, mode: 'dark' | 'light') {
  ...
  if (normalized.includes('fire') || normalized.includes('lava') || normalized.includes('sunset') || normalized.includes('red') || normalized.includes('warm') || normalized.includes('orange') || normalized.includes('sun')) {
    hue = 15; saturation = 85; lightness = 50;
    ...
    hue = 145; saturation = 75; lightness = 45;
    ...
    hue = 320; saturation = 90; lightness = 55;
...
- bgApp, bgCard, bgSecondary, bgInput, border, textPrimary, textSecondary, textMuted, accent, accentHover`;
    ...
    console.warn('Gemini API unavailable. Falling back to deterministic generation.', error.message);
