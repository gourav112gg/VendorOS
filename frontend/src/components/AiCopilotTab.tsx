...
import { Bot, ShieldAlert, Sparkles, RefreshCw, AlertTriangle, CheckCircle2, TrendingUp, Cpu } from 'lucide-react';
...
export const AiCopilotTab: React.FC<AiCopilotTabProps> = ({ company, orders, onNavigateToBilling }) => {
  const access = hasFeatureAccess(company, 'ai_copilot');

  const [selectedOrderId, setSelectedOrderId] = useState<string>('');
  const [analysisResult, setAnalysisResult] = useState<CopilotResult | null>(null);
  ...
  if (!access.hasAccess) {
    return (
      <UpgradePrompt
        ...
        featureDescription="Our core AI differentiator. Automatically analyze complex field work orders to calculate custom operational risk scores, provide clear natural language risk reasons, and suggest immediate mitigation guidelines."
  ...
  const getScoreColor = (score: number) => {
    if (score < 35) return { border: 'border-emerald-500/30', text: 'text-emerald-400', bg: 'bg-emerald-500/10', glow: 'shadow-[0_0_15px_rgba(16,185,129,0.1)]' };
    if (score < 70) return { border: 'border-amber-500/30', text: 'text-amber-400', bg: 'bg-amber-500/10', glow: 'shadow-[0_0_15px_rgba(245,158,11,0.1)]' };
    return { border: 'border-rose-500/30', text: 'text-rose-400', bg: 'bg-rose-500/10', glow: 'shadow-[0_0_15px_rgba(244,63,94,0.15)]' };
  };
      ...
      <div className="bg-[#111111] border border-[#222222] p-6 rounded-sm">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-[#555555] mt-1">
              Select an active field service order to evaluate operational hazards with real-time AI.
            </p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-end">
            ...
            >
              ...
              {orders.map(o => (
                <option key={o.id} value={o.id}>
                  [{o.id}] {o.title} - Valued {company.subscription?.tier === 'free' ? `₹${o.value}` : `₹${o.value}`}
                </option>
              ))}
            </select>
          </div>
          ...
          <button
            ...
            className="w-full sm:w-auto px-5 py-3 bg-white hover:bg-[#E5E5E5] text-black disabled:bg-[#1A1A1A] disabled:text-[#444444] disabled:border-[#222222] border border-transparent rounded-sm text-[10px] font-bold uppercase tracking-widest transition-all shrink-0 cursor-pointer flex items-center justify-center space-x-2"
          >
            {loading ? (
      ...
      <AnimatePresence mode="wait">
        {loading && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="bg-[#111111] border border-[#222222] p-12 text-center rounded-sm space-y-4"
          >
            <div className="flex justify-center">
              <Bot className="w-8 h-8 text-white animate-bounce" />
            </div>
            <p className="text-xs text-white font-mono font-semibold">{loadingTip}</p>
        ...
        {error && (
          ...
          >
            ...
            <div>
              <span className="font-mono font-bold uppercase text-[9px] tracking-wider block">Analysis Failed</span>
        ...
        {analysisResult && selectedOrder && (
          ...
          >
            {/* Risk Gauge Card */}
            <div className={`bg-[#111111] border rounded-sm p-6 flex flex-col items-center justify-center text-center ${getScoreColor(analysisResult.score).border} ${getScoreColor(analysisResult.score).glow}`}>
              ...
              <div className="relative flex items-center justify-center w-28 h-28">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className={`text-3xl font-mono font-extrabold ${getScoreColor(analysisResult.score).text}`}>
              ...
              <span className={`mt-4 inline-flex px-2.5 py-0.5 rounded-sm text-[9px] font-mono uppercase tracking-widest font-bold ${getScoreColor(analysisResult.score).bg} ${getScoreColor(analysisResult.score).text}`}>
                {analysisResult.score < 35 ? 'Low Hazard' : analysisResult.score < 70 ? 'Moderate Alert' : 'Severe Hazard'}
              </span>
            </div>

            {/* AI Explanation and Mitigation Protocol */}
            <div className="lg:col-span-2 bg-[#111111] border border-[#222222] rounded-sm p-6 flex flex-col justify-between">
