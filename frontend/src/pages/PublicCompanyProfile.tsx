...
import { 
  Building, ShieldCheck, Award, MapPin, Send, CheckCircle, Clock, AlertTriangle, AlertCircle, ArrowLeft, ArrowRight, User, Mail, DollarSign
} from 'lucide-react';
...
export const PublicCompanyProfile: React.FC<PublicCompanyProfileProps> = ({ onBackToLogin }) => {
  ...
  const handleSelectCompany = (company: Company) => {
    ...
    const record = scoreRecords[0] || {
      ...
      score: company.id === 'comp_apex' ? 94 : company.id === 'comp_volt' ? 88 : 82,
      factors: {
  ...
  const handleSubmitOrder = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedCompany || !customerName.trim() || !customerEmail.trim() || !title.trim() || !address.trim()) return;
    setSubmitting(true);
    ...
    let clientCust = existingUsers.find(u => u.email.trim().toLowerCase() === customerEmail.trim().toLowerCase() && u.role === 'Customer');
    if (!clientCust) {
      ...
      <div className="bg-[#0D0D0D] border-b border-[#222222] py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center space-x-4">
            {onBackToLogin && (
              <button 
                ...
                className="p-2 border border-[#222222] bg-[#111111] hover:bg-[#222222] rounded-sm text-[#888888] hover:text-white transition-all cursor-pointer"
              >
            ...
            <div>
              ...
              <p className="text-xs text-[#666666] uppercase tracking-wider font-mono mt-0.5">
          ...
          <div className="flex items-center space-x-3">
            <span className="text-xs font-mono text-[#666666] uppercase">Select Company:</span>
            <div className="flex bg-[#111111] border border-[#222222] p-1 rounded-sm gap-1">
              {companies.map(c => (
                <button
                  key={c.id}
                  onClick={() => handleSelectCompany(c)}
                  className={`px-3 py-1.5 rounded-sm text-[10px] font-mono font-bold uppercase tracking-wider transition-all cursor-pointer ${
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="bg-[#111111] border border-[#222222] p-6 rounded-sm">
                <span className="text-[9px] font-mono text-[#666666] uppercase tracking-widest block mb-1">Provider Overview</span>
                <h2 className="text-3xl font-serif italic text-white font-light">{selectedCompany.name}</h2>
                ...
                <p className="text-xs text-[#888888] leading-relaxed mt-4">
                  A fully licensed contracting firm utilizing VendorOS tracking systems to ensure pristine compliance, 
                  live worker dispatch transparency, and automated material updates.
                </p>

                <div className="mt-6 pt-6 border-t border-[#222222] flex items-center justify-between">
                  <div>
                    <span className="text-[10px] font-mono text-[#666666] block uppercase tracking-wider">Operational Registry</span>
                    <span className="text-xs text-[#888888] font-mono mt-0.5 block">Registered {new Date(selectedCompany.createdAt).toLocaleDateString()}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-mono text-[#666666] block uppercase tracking-wider text-right">Service Rating</span>
                    <span className="text-sm text-emerald-400 font-bold block text-right font-mono">★★★★★ 4.9/5</span>
                  </div>
                </div>
              </div>
              ...
              {trustScore && (
                <div className="bg-[#111111] border border-[#222222] p-6 rounded-sm">
                  <div className="flex justify-between items-center mb-4">
                    <span className="text-[9px] font-mono text-[#666666] uppercase tracking-widest">Verified Trust Score</span>
                  ...
                  <div className="flex items-center space-x-6 pb-6 border-b border-[#222222]">
                    <div className="relative flex items-center justify-center w-20 h-20 rounded-full border border-emerald-950/40 bg-[#0A2215]/30">
                      <span className="text-3xl font-mono text-emerald-400 font-bold">{trustScore.score}</span>
                      <span className="text-[10px] text-emerald-600 font-semibold absolute bottom-2">/100</span>
                    </div>
                    <div className="flex-grow">
                      <span className="text-sm font-semibold text-white block">Excellent Rating</span>
                      <p className="text-xs text-[#666666] mt-1 leading-snug">
                        Calculated from real-time execution telemetry. High score denotes reliable scheduling, safe inventory caches, and rapid fulfillment.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="pt-6 space-y-4 font-mono text-[10px] uppercase tracking-wider">
                    <div>
                      <div className="flex justify-between text-[#888888] mb-1">
                        ...
                        <span className="text-white font-bold">{trustScore.factors.orderCompletionRate}%</span>
                      </div>
                      <div className="w-full bg-[#1A1A1A] h-1 rounded-full overflow-hidden">
                        <div className="bg-emerald-400 h-full rounded-full" style={{ width: `${trustScore.factors.orderCompletionRate}%` }}></div>
                      </div>
                    </div>
                    <div>
                      <div className="flex justify-between text-[#888888] mb-1">
                        <span>Material Cache Level</span>
                        <span className="text-white font-bold">{trustScore.factors.inventoryLevelRating}%</span>
                      </div>
                      <div className="w-full bg-[#1A1A1A] h-1 rounded-full overflow-hidden">
                        <div className="bg-blue-400 h-full rounded-full" style={{ width: `${trustScore.factors.inventoryLevelRating}%` }}></div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {/* Operational Domains */}
              <div className="bg-[#111111] border border-[#222222] p-6 rounded-sm">
                <span className="text-[9px] font-mono text-[#666666] uppercase tracking-widest block mb-4">Active Service Domains ({domains.length})</span>
                {domains.length === 0 ? (
                  <p className="text-xs text-[#666666] font-mono italic">No active domains reported.</p>
                ) : (
                  <div className="space-y-3">
                    {domains.map(d => (
                      <div 
                        key={d.id} 
                        className="p-3 bg-[#0D0D0D] border border-[#222222] rounded-sm flex items-center justify-between"
                      >
                        <div>
                          <span className="text-xs font-bold text-white block">{d.name}</span>
                          <span className="text-[10px] text-[#666666] font-mono uppercase tracking-wider mt-0.5 block">{d.type}</span>
                        </div>
            ...
            <div className="lg:col-span-7">
              <div className="bg-[#111111] border border-[#222222] rounded-sm overflow-hidden">
                <div className="p-6 bg-[#0A0A0A] border-b border-[#222222] flex justify-between items-center">
                  <div>
                    <h3 className="text-lg font-serif italic text-white font-light">Self-Serve Order Intake</h3>
                  ...
                  {selectedCompany.minOrderValue && (
                    <div className="text-right">
                      <span className="text-[9px] font-mono text-[#666666] block uppercase tracking-wider">Min Threshold</span>
                      <span className="text-xs text-amber-400 font-mono font-bold">₹{selectedCompany.minOrderValue.toLocaleString()} / $500</span>
                    </div>
                  )}
                </div>
                ...
                {successOrder ? (
                  <div className="p-10 text-center space-y-6">
                    ...
                    <div>
                      <h4 className="text-xl font-serif italic text-white">Order Placed Successfully!</h4>
                      <p className="text-xs text-[#888888] max-w-md mx-auto mt-2 leading-relaxed">
                        Your request has been successfully stored. Our management staff has been notified to construct your delivery stages and schedule technicians.
                      </p>
                    </div>
                    ...
                    <div className="bg-[#0D0D0D] border border-[#222222] p-5 max-w-sm mx-auto rounded-sm text-left font-mono text-xs space-y-3">
                      ...
                      <div className="flex justify-between border-b border-[#1A1A1A] pb-2 text-[#666666]">
                        <span>EST. VALUE</span>
                        <span className="text-white font-bold">₹{successOrder.value.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between border-b border-[#1A1A1A] pb-2 text-[#666666]">
                        ...
                        {successOrder.belowMinimumThreshold ? (
                          <span className="text-amber-400 font-bold uppercase tracking-wider flex items-center gap-1 text-[10px]">
                            <AlertCircle className="w-3.5 h-3.5" /> Gated (Below Min)
                          </span>
                        ) : (
                          <span className="text-emerald-400 font-bold uppercase tracking-wider text-[10px]">Auto-Accepted</span>
                        )}
                      </div>
                      {successOrder.belowMinimumThreshold && (
                        <p className="text-[10px] text-amber-400/80 leading-relaxed pt-1 font-sans">
                          * Note: This order is below the company's configured minimum threshold of <strong>₹{(selectedCompany.minOrderValue || 2000).toLocaleString()}</strong>. It is held on the Owner's desk for acceptance before dispatch.
                        </p>
                      )}
                    </div>
                    ...
                    <div className="pt-2">
                      <button
                        ...
                        className="px-5 py-2.5 bg-white hover:bg-[#E5E5E5] text-black text-xs font-bold font-mono uppercase tracking-wider rounded-sm transition-all cursor-pointer"
                      >
                  ...
                  <form onSubmit={handleSubmitOrder} className="p-6 space-y-5">
                    ...
                    <div className="bg-[#0D0D0D] p-4 rounded-sm border border-[#222222] space-y-4">
                      <span className="text-[9px] font-mono text-[#666666] uppercase tracking-widest block font-bold">Customer Contact Identity</span>
                    ...
                    <div className="space-y-4">
                      <span className="text-[9px] font-mono text-[#666666] uppercase tracking-widest block font-bold">Job Description</span>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                          <label className="block text-[10px] font-mono font-medium text-[#888888] uppercase tracking-wider mb-1.5 flex justify-between">
                            ...
                            {selectedCompany.minOrderValue && (
                              <span className="text-[9px] text-[#555555] font-mono">Min: ₹{selectedCompany.minOrderValue}</span>
                            )}
                          </label>
                          <div className="relative">
                            <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-[#444444]">
                    ...
                    {selectedCompany.minOrderValue && value < selectedCompany.minOrderValue && (
                      <div className="p-3 bg-[#2D1D0F] border border-amber-950/40 rounded-sm text-amber-400 text-[11px] leading-relaxed flex items-start space-x-2.5">
                        ...
                        <div>
                          <strong>Below Minimum Order Threshold Warning:</strong> Your proposed budget of <strong>₹{value.toLocaleString()}</strong> is below the provider's threshold of <strong>₹{selectedCompany.minOrderValue.toLocaleString()}</strong>.
                          Your order will require manual desk approval by the Owner before staging and dispatch.
                        </div>
                      </div>
                    )}
                    ...
                    <div className="pt-4 border-t border-[#222222] flex justify-end">
                      <button
                        type="submit"
                        disabled={submitting}
                        className="px-6 py-3 bg-white hover:bg-[#E5E5E5] text-black font-bold font-mono text-xs uppercase tracking-wider rounded-sm transition-all flex items-center space-x-2 cursor-pointer disabled:opacity-30"
                      >
                        {submitting ? (
