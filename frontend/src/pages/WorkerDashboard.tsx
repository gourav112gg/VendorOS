...
export const WorkerDashboard: React.FC = () => {
  ...
  const [activeRecordingStageId, setActiveRecordingStageId] = useState<
    string | null
  >(null);
  ...
  const handleUpdateSubstageStatus = (
    orderId: string,
    stageId: string,
  ...
  const startSpeechRecognition = (stageId: string) => {
    ...
    if (!SpeechRecognition) {
      alert(
        "Local browser Speech Recognition API is not supported in this environment. Please type or use our Quick Voice presets in the Assist panel!",
      );
      return;
    }
    ...
    try {
      ...
      recognition.onerror = (event: any) => {
        ...
        alert(
          `Mic input error: ${event.error}. Please type in our smart assist panel instead!`,
        );
      };
  ...
  const processVoiceCommand = (transcript: string, stageId: string) => {
    ...
    const updatedChecklist = stage.checklist.map((item) => {
      ...
      const isMatch =
        directMatch ||
        (keywords.length > 0 && kwMatchCount >= Math.min(2, keywords.length));

      if (isMatch && !item.completed) {
        matchedAny = true;
        setVoiceLog((prev) => [
          ...prev,
          `[Auto-Verified]: Checked "${item.text}"`,
        ]);
    ...
    if (matchedAny) {
      ...
      setVoiceLog((prev) => [
        ...prev,
        `[No Match] Speech received: "${transcript}"`,
      ]);
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    const rows = filteredOrders.map((job) => {
      const companyName =
        companies.find((c) => c.id === job.companyId)?.name || "Vendor Company";
      return [
        job.id,
        job.title,
        companyName,
    ...
    link.setAttribute(
      "download",
      `assigned_orders_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  const totalAssignedValue = assignedOrders.reduce(
    (sum, o) => sum + o.value,
    0,
  );
  ...
  const completedTodayOrders = assignedOrders.filter(
    (job) =>
      job.stage === "Completed" && isToday(job.completedAt || job.createdAt),
  );
  const completedTodayCount = completedTodayOrders.length;
  const completedTodayValue = completedTodayOrders.reduce(
    (sum, o) => sum + o.value,
    0,
  );
          ...
          <div className="lg:col-span-3 space-y-4">
            <div>
              <h1 className="text-2xl font-serif italic font-light text-white tracking-tight flex items-center">
                <Briefcase className="w-6 h-6 mr-2 text-[#666666]" />
                Service Hub
              </h1>
            ...
            <div className="flex flex-col bg-[#111111] p-1.5 border border-[#222222] rounded-sm space-y-1 shadow-md">
              {(
                [
                  { id: "jobs", label: "My Jobs", icon: null },
                  {
                    id: "settings",
                    label: "Settings",
                ...
                >
                  {activeTab === t.id && (
                    <motion.span
                      layoutId="vos-worker-sidebar-pill"
            ...
            <div className="flex items-center justify-between space-x-3 bg-[#111111] px-3 py-2 border border-[#222222] rounded-sm select-none shadow-sm">
              <div className="flex items-center space-x-2">
                <RefreshCw
                  className={`w-3.5 h-3.5 ${autoRefresh ? "text-emerald-400 animate-spin" : "text-[#666666]"}`}
        ...
        >
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 pb-4 border-b border-[#222222]">
              <div>
                ...
                <p className="text-[#666666] mt-1 text-xs font-mono uppercase tracking-widest">
                  Worker Dashboard • View assigned dispatches, update service
                  stages, and log job details.
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-4 mt-4 md:mt-0">
                {/* User-controlled Auto Refresh Toggle Switch */}
                <div className="flex items-center space-x-3 bg-[#111111] px-3 py-2 border border-[#222222] rounded-sm select-none shadow-sm">
                  <div className="flex items-center space-x-2">
                    <RefreshCw
                      className={`w-3.5 h-3.5 ${autoRefresh ? "text-emerald-400 animate-spin" : "text-[#666666]"}`}
                      style={
                        autoRefresh ? { animationDuration: "3s" } : undefined
                      }
                    />
                    <span className="text-[10px] font-mono font-bold text-[#888888] uppercase tracking-wider">
                      Auto Refresh (60s)
                  ...
                  <button
                    id="auto-refresh-toggle-horizontal"
                    onClick={() => setAutoRefresh(!autoRefresh)}
                    className={`relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                      autoRefresh ? "bg-emerald-500" : "bg-[#222222]"
                    }`}
          ...
          {activeTab === "settings" ? (
            ...
            <div className="space-y-8 animate-fade-in">
              {/* Summary Section */}
              <div className="vos-stagger grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Card 1: Assigned Orders */}
                <motion.div
                  whileHover={{ y: -3 }}
                  transition={{ type: "spring", stiffness: 300, damping: 22 }}
                  className="bg-[#111111] p-6 rounded-sm border border-[#222222] flex items-start justify-between shadow-md hover:border-[#333333]"
                >
                  <div>
                    <span className="text-[10px] font-mono font-medium text-[#666666] uppercase tracking-widest block">
                      Total Assigned Orders
                    </span>
                    <span className="text-4xl font-sans font-extrabold text-white mt-2 block">
                      {totalAssignedCount}
                    </span>
                    <span className="text-xs font-mono text-[#888888] mt-1.5 inline-block">
                      Assigned Value:{" "}
                      <span className="text-white font-bold">
                        {formatIndianCurrency(totalAssignedValue)}
                      </span>
                    </span>
                  </div>
                  ...
                  <div>
                    ...
                    <span className="text-xs font-mono text-emerald-400 mt-1.5 inline-block">
                      ...
                      <span className="text-emerald-400 font-bold">
                        {formatIndianCurrency(completedTodayValue)}
                      </span>
                    </span>
                  </div>
              ...
              {assignedOrders.length === 0 ? (
                <div className="bg-[#111111] border border-[#222222] rounded-sm p-12 text-center max-w-2xl mx-auto mt-6">
                  ...
                  <h2 className="text-xl font-serif italic font-light text-white">
                    You are fully caught up!
                  </h2>
                  <p className="text-xs text-[#666666] max-w-sm mx-auto mt-2">
                    No service orders are currently dispatched to you. Relax or
                    notify your dispatch manager.
                  </p>
                </div>
              ) : (
                        ...
                        filteredOrders.map((job) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <p
                                className={`text-[10px] font-mono font-medium uppercase tracking-widest mt-1.5 flex items-center ${isSelected ? "text-emerald-400" : "text-[#888888]"}`}
                              >
                              ...
                              <p
                                className={`text-xs mt-2 flex items-center ${isSelected ? "text-white/80" : "text-[#666666]"}`}
                              >
                              ...
                              <div className="mt-4 pt-3 border-t border-[#222222] border-dashed flex justify-between items-center text-xs font-semibold">
                                ...
                                <span
                                  className={`font-mono ${isSelected ? "text-white" : "text-[#E5E5E5]"}`}
                                >
                    ...
                    {selectedOrder ? (
                      ...
                      >
                        ...
                        <div className="p-6 space-y-6">
                          ...
                          {/* Customer Address */}
                          <div>
                            <span className="text-[10px] font-mono font-medium text-[#666666] block uppercase tracking-widest mb-1.5">
                              Site Location
                            </span>
                          ...
                          <div>
                            <span className="text-[10px] font-mono font-medium text-[#666666] block uppercase tracking-widest mb-3">
                              Service Stage
                            </span>
                              ...
                              {(
                                ...
                                return (
                                  <div key={stage} className="text-center">
                                    ...
                                    ></div>
                                    <span
                                      className={`text-[10px] font-bold block uppercase tracking-widest ${
                                        isActive
                                          ? "text-white"
                                          : isCompleted
                                            ? "text-emerald-500"
                          ...
                          <div className="pt-6 border-t border-[#222222] space-y-4">
                            ...
                            selectedOrder.stages.length === 0 ? (
                              <div className="p-6 text-center text-xs font-mono text-[#555555] bg-[#0D0D0D] border border-dashed border-[#222222] rounded-sm">
                                No custom delivery stages defined by dispatch
                                manager for this order. Please advance top-level
                                statuses manually below.
                              </div>
                            ) : (
                              <div className="space-y-4">
                                {selectedOrder.stages.map((stg, sIdx) => {
                                  const isAssignedToMe =
                                    stg.assignedWorkerId === user.id;
                                  const isRecordingThis =
                                    isRecording &&
                                    activeRecordingStageId === stg.id;
                                    ...
                                    >
                                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
                                        ...
                                        <div className="flex items-center space-x-2">
                                          ...
                                          <span
                                            className={`text-[10px] font-mono px-2 py-0.5 rounded-sm uppercase tracking-wider font-bold ${
                                        ...
                                        stg.status === "In Progress" && (
                                          <div className="mt-4 p-3 bg-[#070707] border border-[#1A1A1A] rounded-sm space-y-3">
                                            ...
                                            <div className="space-y-1 pt-1">
                                              <span className="text-[8px] font-mono text-[#444444] uppercase block">
                                                Quick Speak Assist Simulations
                                                (1-click test):
                                              </span>
                                                ...
                                                {stg.checklist
                                                  ...
                                                  .map((item) => (
                                                    <button
                                                      ...
                                                      onClick={() =>
                                                        processVoiceCommand(
                                                          `I have verified and completed the step to ${item.text}`,
                                                          stg.id,
                                                        )
                                                      }
                          ...
                          <div className="pt-6 border-t border-[#222222]">
                            {selectedOrder.stage === "Completed" ? (
                              <div className="bg-[#0D2A1D] border border-emerald-950/40 rounded-sm p-4 text-center flex flex-col items-center">
                                ...
                                <h4 className="font-bold text-emerald-400 text-sm font-mono uppercase tracking-widest">
                                  Job Successfully Completed!
                                </h4>
                                <p className="text-xs text-emerald-500/80 mt-1">
                                  Completion logs saved in the portal database.
                                </p>
                              </div>
                            ) : (
                              <div className="space-y-4">
                                <h4 className="text-[10px] font-mono font-bold text-[#666666] uppercase tracking-widest">
                                  Advance Job Stage:
                                </h4>
                                ...
                                <div className="flex flex-wrap gap-3">
                                  {selectedOrder.stage === "Scheduled" && (
                                    <button
                                      onClick={() =>
                                        handleUpdateStage("Dispatched")
                                      }
                                      disabled={statusUpdating}
                                      className="px-5 py-2.5 bg-white hover:bg-[#F0EAD8] text-black rounded-sm font-bold text-[10px] uppercase tracking-widest transition-all flex items-center cursor-pointer"
                                    >
                                      {statusUpdating
                                        ? "Updating..."
                                        : "Start Driving"}
                                ...
                                {showNotesForm && (
                                  ...
                                  >
                                    <div className="flex items-center space-x-1.5 text-white">
                                      ...
                                      <h5 className="font-bold text-[10px] uppercase tracking-widest font-mono text-[#666666]">
                                        Job Completion Log
                                      </h5>
                                    </div>
