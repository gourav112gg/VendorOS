...
import { 
  Plus, ClipboardList, CheckCircle, Clock, MapPin, Building, FileText, Send, X, AlertCircle, Settings
} from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 pb-4 border-b border-[#222222]">
        <div>
          ...
          <p className="text-[#666666] mt-1 text-xs font-mono uppercase tracking-widest">
            Customer Dashboard • Submit requests, follow order progress, and view completed jobs.
          </p>
        </div>
        ...
        <div className="flex items-center space-x-3 mt-4 md:mt-0">
          ...
          <div className="flex bg-[#111111] p-1 border border-[#222222] rounded-sm space-x-1">
            ...
            >
              <span>Requests</span>
              <ShortcutBadge tab="requests" className="opacity-0 group-hover:opacity-100 transition-opacity ml-1.5" />
            </button>
            <button
          ...
          {activeTab === 'requests' && (
            <button
              ...
              className="flex items-center justify-center space-x-1.5 px-5 py-3 bg-white hover:bg-[#E5E5E5] text-black rounded-sm text-[10px] font-bold uppercase tracking-widest transition-all cursor-pointer"
            >
      ...
      {activeTab === 'settings' ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className={`lg:col-span-5 space-y-4 ${
            ...
            <h3 className="text-[10px] font-mono font-bold text-[#666666] uppercase tracking-widest mb-2">My Service Tickets ({orders.length})</h3>
            {orders.length === 0 ? (
              <div className="bg-[#111111] border border-[#222222] rounded-sm p-8 text-center text-[#666666]">
                ...
                <p className="font-medium text-white text-sm">No service tickets logged.</p>
                <p className="text-xs text-[#666666] mt-1 font-mono">Click "Submit Request" to start.</p>
              </div>
            ) : (
              <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
                {orders.map((order) => {
                  const isSelected = selectedOrder?.id === order.id;
                  const companyName = companies.find(c => c.id === order.companyId)?.name || 'Vendor Company';
                  return (
                    <button
                      key={order.id}
                    ...
                    >
                      ...
                      <p className={`text-xs mt-1.5 flex items-center font-medium font-mono uppercase tracking-wider ${isSelected ? 'text-emerald-400' : 'text-emerald-500'}`}>
                      ...
                      <div className="mt-4 pt-3 border-t border-[#222222] border-dashed flex justify-between items-center text-xs">
                        <span className={isSelected ? 'text-white/80' : 'text-[#888888]'}>
                          Estimated: <span className="font-mono font-bold text-white">{formatCurrency(order.value)}</span>
                        </span>
                        {order.workerId && (
                          <span className={`font-mono ${isSelected ? 'text-white' : 'text-[#888888]'}`}>
          ...
          <div className="lg:col-span-7">
            {selectedOrder ? (
              ...
              >
                <div className="p-6 bg-[#0A0A0A] border-b border-[#222222]">
                  ...
                  <p className="text-xs text-emerald-400 font-semibold mt-1 flex items-center font-mono uppercase tracking-wider">
                    ...
                    Assigned Provider: {companies.find(c => c.id === selectedOrder.companyId)?.name || 'Vendor'}
                  </p>
                </div>
                ...
                <div className="p-6 space-y-6">
                  {/* Status timelines */}
                  <div>
                    <span className="text-[10px] font-mono font-medium text-[#666666] block uppercase tracking-widest mb-4">Order Status</span>
                    ...
                    <div className="space-y-6 relative pl-6 border-l-2 border-[#222222]">
                      ...
                      <div className="relative">
                        <div className={`absolute -left-[31px] top-0.5 w-4 h-4 rounded-full border-2 ${
                          selectedOrder.stage !== 'Completed' ? 'bg-[#333333] border-[#222222]' : 'bg-emerald-500 border-emerald-950'
                        }`}></div>
                        <div>
                          <span className="text-xs font-bold text-white block">Request Submitted</span>
                          <span className="text-xs text-[#666666]">Request received. Awaiting dispatch assignment.</span>
                        </div>
                      </div>
                      ...
                      <div className="relative">
                        <div className={`absolute -left-[31px] top-0.5 w-4 h-4 rounded-full border-2 ${
                          ['Scheduled', 'Dispatched', 'In Progress', 'Completed'].includes(selectedOrder.stage)
                            ? (selectedOrder.stage === 'Completed' ? 'bg-emerald-500 border-emerald-950' : 'bg-[#333333] border-[#222222]')
                            : 'bg-[#111111] border-[#222222]'
                        }`}></div>
                        <div>
                          <span className={`text-xs font-bold block ${['Scheduled', 'Dispatched', 'In Progress', 'Completed'].includes(selectedOrder.stage) ? 'text-white' : 'text-[#444444]'}`}>
                            Technician Assigned
                          </span>
                          {selectedOrder.workerId ? (
                            <span className="text-xs text-[#888888]">
                              Technician <strong className="text-white">{selectedOrder.workerName}</strong> has been allocated to your ticket.
                            </span>
                          ) : (
                            <span className="text-xs text-[#666666]">Waiting for a technician to be assigned.</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="relative">
                        <div className={`absolute -left-[31px] top-0.5 w-4 h-4 rounded-full border-2 ${
                          ['In Progress', 'Completed'].includes(selectedOrder.stage)
                            ? (selectedOrder.stage === 'Completed' ? 'bg-emerald-500 border-emerald-950' : 'bg-[#333333] border-[#222222]')
                            : 'bg-[#111111] border-[#222222]'
                        }`}></div>
                        <div>
                          <span className={`text-xs font-bold block ${['In Progress', 'Completed'].includes(selectedOrder.stage) ? 'text-white' : 'text-[#444444]'}`}>
                            Work In Progress
                          </span>
                          <span className="text-xs text-[#888888]">
                      ...
                      <div className="relative">
                        <div className={`absolute -left-[31px] top-0.5 w-4 h-4 rounded-full border-2 ${
                          selectedOrder.stage === 'Completed' ? 'bg-emerald-500 border-emerald-950' : 'bg-[#111111] border-[#222222]'
                        }`}></div>
                        <div>
                          <span className={`text-xs font-bold block ${selectedOrder.stage === 'Completed' ? 'text-white' : 'text-[#444444]'}`}>
                            Service Completed
                          </span>
                          <span className="text-xs text-[#888888]">
                            {selectedOrder.stage === 'Completed' 
                  ...
                  <div className="border-t border-[#222222] pt-6 space-y-4">
                    <div>
                      <span className="text-[10px] font-mono font-medium text-[#666666] block uppercase tracking-widest mb-1">Service Location</span>
                    ...
                    <div>
                      <span className="text-[10px] font-mono font-medium text-[#666666] block uppercase tracking-widest mb-1">Description</span>
              ...
              <div className="bg-[#111111] rounded-sm border border-[#222222] p-12 text-center text-[#666666] text-xs font-mono">
                Please select a service ticket from the list to track its real-time progress.
              </div>
            )}
          </div>
        </div>
          ...
          >
            <div className="p-6 border-b border-[#222222] flex items-center justify-between bg-[#0A0A0A]">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="font-serif italic font-light text-lg text-white">Submit New Service Request</h3>
              </div>
              <button 
            ...
            {formSuccess ? (
              <div className="p-12 text-center space-y-3">
                ...
                <h4 className="font-sans font-bold text-xl text-white">Request Dispatched!</h4>
                <p className="text-sm text-[#888888]">Your ticket is logged and active in the system.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmitOrder} className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-[10px] font-mono font-medium text-[#666666] uppercase tracking-widest mb-1.5">
                    Estimated Budget ({preferences.currency === 'INR' ? '₹' : '$'}) *
                  </label>
                  <input
                    type="number"
                    required
                ...
                <div className="pt-4 border-t border-[#222222] flex justify-end space-x-3">
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="px-5 py-2 bg-white hover:bg-[#E5E5E5] text-black rounded-sm text-xs font-bold uppercase tracking-widest transition-all flex items-center cursor-pointer"
                  >
