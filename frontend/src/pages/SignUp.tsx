  ...
  return (
    <div className="relative min-h-screen bg-[#0B1420] text-[#EDEDED] flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 overflow-hidden">
      ...
      <div className="relative z-10 mt-8 sm:mx-auto sm:w-full sm:max-w-xl">
        ...
        >
          {/* Sign Up Mode Tabs */}
          <div className="grid grid-cols-3 bg-[#0B1420] p-1 border border-[#223245] rounded-sm mb-6 gap-1">
            <button
              type="button"
              onClick={() => {
                setSignUpMode("owner");
              ...
              className={`py-2 px-1 text-[10px] font-bold rounded-sm uppercase tracking-wider transition-all truncate text-center ${
                signUpMode === "owner"
                  ? "bg-[] text-[#7FA0C4] font-extrabold"
                  : "text-[#7C8B9C] hover:text-[#c9e830]"
              }`}
            >
              Start Company
            </button>
            <button
              type="button"
              onClick={() => {
                setSignUpMode("employee");
                setError("");
              }}
              className={`py-2 px-1 text-[10px] font-bold rounded-sm uppercase tracking-wider transition-all truncate text-center ${
                signUpMode === "employee"
                  ? "bg-[] text-[#7FA0C4] font-extrabold"
                  : "text-[#7C8B9C] hover:text-[]"
              }`}
            >
              Join Vendor
            </button>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-mono font-medium text-[#7C8B9C] uppercase tracking-widest mb-1.5">
                  Full Name
                </label>
                <div className="relative rounded-sm shadow-xs">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 rounded-sm border border-[#223245] bg-[#0A1119] text-[#EDEDED] focus:outline-none focus:ring-1 focus:ring-[#C9662E] focus:border-[#C9662E] text-xs font-mono placeholder-[#45566B]"
                  />
                </div>
              </div>
            ...
            {signUpMode === "owner" && (
              <motion.div
                ...
                className="space-y-4 pt-4 border-t border-[#1B2836]"
              >
                <div>
                  <label className="block text-[10px] font-mono font-medium text-[#7C8B9C] uppercase tracking-widest mb-1.5">
                    Company Name
                  ...
                  <div className="relative rounded-sm shadow-xs">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-10 py-2.5 rounded-sm border border-[#223245] bg-[#0A1119] text-[#EDEDED] focus:outline-none focus:ring-1 focus:ring-[#C9662E] focus:border-[#C9662E] text-xs font-mono placeholder-[#45566B]"
                    />
                    ...
                    {isCompanyAvailable !== null && (
                      <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
                        {isCompanyAvailable ? (
                          <CheckCircle
                            className="w-5 h-5 text-[#8FC1FF]"
                            title="Company Name Available!"
                          />
                        ) : (
                          <XCircle
                            className="w-5 h-5 text-[#B23A1B]"
                  ...
                  {isCompanyAvailable !== null && (
                    <p
                      className={`text-xs mt-1.5 font-semibold ${isCompanyAvailable ? "text-[#8FC1FF]" : "text-[#B23A1B]"}`}
                    >
                      {isCompanyAvailable
                        ? "✓ Beautiful! This company name is available for registration."
                        : "✗ Warning: This company name is already registered."}
                    </p>
                  )}
                </div>
              </motion.div>
            ...
            {signUpMode === "employee" && (
              ...
              >
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2.5 rounded-sm border border-[#223245] bg-[#0A1119] text-[#EDEDED] focus:outline-none focus:ring-1 focus:border-[#C9662E] text-xs font-mono"
                    >
                  ...
                  <div>
                    ...
                    <div className="w-full px-3 py-2.5 rounded-sm bg-[#0A1119] border border-[#223245] text-[#A9B4C0] text-xs font-semibold flex items-center">
                      <span className="w-2 h-2 bg-[#C9662E] rounded-full mr-2"></span>
                      Instant Access — Flagged for Owner Review
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="border border-[#223245] rounded-sm p-3 bg-[#0B1420] space-y-2">
                    <div className="relative rounded-sm">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-3 py-1.5 rounded-sm border border-[#223245] bg-[#0A1119] text-[#EDEDED] focus:outline-none focus:ring-1 focus:border-[#C9662E] text-xs font-mono placeholder-[#45566B]"
                      />
                    </div>

                    <div className="max-h-24 overflow-y-auto space-y-1 mt-1 pr-1">
                      {filteredCompanies.length === 0 ? (
                        <p className="text-[11px] text-[#7C8B9C] italic text-center py-2">
                          No vendor companies match your query.
                        </p>
                      ) : (
                          ...
                          return (
                            <button
                              ...
                              className={`w-full text-left px-3 py-1.5 rounded-sm text-xs font-semibold flex justify-between items-center transition-colors ${
                                isSelected
                                  ? "bg-[] text-[#7FA0C4] font-extrabold"
                                  : "hover:bg-[#1B2836] text-[#A9B4C0] bg-transparent hover:text-[]"
                              }`}
                            >
                              <span>{c.name}</span>
                              <span
                                className={`text-[10px] font-mono ${isSelected ? "text-[#7FA0C4]" : "text-[#B5A88A]"}`}
                              >
                                {c.id}
                              </span>
                            </button>
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-sm border border-[#223245] bg-[#0A1119] text-[#EDEDED] focus:outline-none focus:ring-1 focus:border-[#C9662E] text-xs font-mono placeholder-[#45566B]"
                  />
                </div>
              </motion.div>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full flex justify-center items-center py-3 px-4 bg-[#C9662E] hover:bg-[#B85826] text-[#FFFCF5] text-[11px] font-bold uppercase tracking-widest rounded-sm transition-all focus:outline-none disabled:opacity-50 cursor-pointer"
            >
              {loading ? (
            ...
            <button
              ...
              disabled={loading}
              className="w-full flex justify-center items-center py-3 px-4 bg-[#111B29] hover:bg-[#182636] text-[] text-[11px] font-bold uppercase tracking-widest rounded-sm border border-[#223245] hover:border-[] transition-all duration-150 disabled:opacity-50 cursor-pointer"
            >
              <svg
          ...
          <div className="mt-6 text-center text-xs">
            <span className="text-[#7C8B9C]">Already registered? </span>
            <button
              onClick={onNavigateToLogin}
              className="font-bold uppercase tracking-wider text-[10px] text-[] hover:text-[#C9662E] underline focus:outline-none cursor-pointer"
            >
              Sign In Instead
            </button>
          </div>
