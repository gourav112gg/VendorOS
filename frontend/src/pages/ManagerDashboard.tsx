...
export const ManagerDashboard: React.FC = () => {
  ...
  // Tab states
  const [activeSubTab, setActiveSubTab] = useState<'overview' | 'orders' | 'domains' | 'team' | 'copilot' | 'invoices' | 'trust-score' | 'analytics' | 'billing' | 'settings' | 'activity'>('overview');
  ...
  useEffect(() => {
    ...
    const loadData = () => {
      ...
      if (editingOrderStages) {
        const freshEditing = freshOrders.find(o => o.id === editingOrderStages.id);
        if (freshEditing) {
  ...
  useEffect(() => {
    const handleNav = (e: Event) => {
      ...
      const validTabs = [
        'overview', 'orders', 'domains', 'team', 'copilot', 'invoices', 'trust-score', 'analytics', 'billing', 'settings', 'activity'
      ];
  ...
  const isWorkerBusy = (workerId: string) => {
    return orders.some(o => 
      o.stage !== 'Completed' && 
      (o.stages || []).some(s => s.assignedWorkerId === workerId && s.status === 'In Progress')
    );
  };
      ...
      <div className={preferences.navStyle === 'sidebar' ? 'grid grid-cols-1 lg:grid-cols-12 gap-8' : ''}>
        ...
        <div className={preferences.navStyle === 'sidebar' ? 'lg:col-span-9 space-y-8' : 'space-y-8'}>
          ...
          {activeSubTab === 'overview' && (
            ...
            >
              ...
              <div className="bg-[#0D1D2D] border border-blue-950/40 rounded-sm p-6">
                <h3 className="font-serif italic font-light text-lg text-white">Manager Profile Privileges</h3>
                <p className="text-xs text-[#888888] mt-2 leading-relaxed max-w-3xl">
                  As a Manager, you have permission to view company-wide operational domains, 
                  view registered team members, and fully control technician dispatches for service requests. 
                  Administrative edits to domains or personnel removals are reserved exclusively for the company Owner.
                </p>
              </div>
            </motion.div>
          )}
              ...
              {!editingOrderStages ? (
                ...
                >
                  <div className="p-6 border-b border-[#222222] bg-[#0A0A0A] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                      <h2 className="text-lg font-serif italic font-light text-white">Dispatcher Command Panel</h2>
                      <p className="text-xs text-[#666666] mt-1">
                        Select any assigned field order to build its custom service execution stages and verify checklists.
                      </p>
                    </div>

                    <div className="flex bg-[#0D0D0D] p-1 border border-[#222222] rounded-sm self-start sm:self-center font-mono">
                      <button
                        onClick={() => setSelectedTemplateId('') /* Reset and just filter */}
                  ...
                  {orders.length === 0 ? (
                    <div className="p-12 text-center text-[#666666] text-xs font-mono">No active service requests found.</div>
                  ) : (
                            ...
                            return (
                              <tr 
                                ...
                                className={`hover:bg-[#0D0D0D] transition-colors cursor-pointer ${
                                  isMine ? 'bg-[#1D1D2D]/10 hover:bg-[#1D1D2D]/20' : ''
                                }`}
                                onClick={() => setEditingOrderStages(order)}
                              >
                                <td className="px-6 py-4 font-mono text-xs text-[#666666]">{order.id}</td>
                                ...
                                <td className="px-6 py-4">
                                  {order.managerId ? (
                                    ...
                                    <span className="font-mono text-xs text-[#444444] italic">Unassigned</span>
                                  )}
                                </td>
                                <td className="px-6 py-4">
                                  {order.workerId ? (
                                ...
                                <td className="px-6 py-4 text-[#888888] font-mono font-semibold">{stageCount} stages</td>
                                <td className="px-6 py-4">
                                ...
                                <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                                  <button
                                    onClick={() => setEditingOrderStages(order)}
                                    className="text-[10px] uppercase font-mono tracking-wider bg-white hover:bg-[#E5E5E5] text-black font-semibold px-2.5 py-1.5 rounded-sm transition-colors cursor-pointer"
                                  >
                                    Manage Stages
                                  </button>
                                </td>
                ...
                >
                  ...
                  <div className="bg-[#111111] border border-[#222222] p-6 rounded-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                      ...
                      <h2 className="text-xl font-serif italic text-white font-light">Stage Builder & Checklist Protocol</h2>
                      <p className="text-xs text-[#666666] mt-1 font-mono">
                        Active Order: <strong className="text-slate-300">{editingOrderStages.title}</strong> • ID: {editingOrderStages.id}
                      </p>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <span className="font-mono text-xs bg-[#0D2A1D] text-emerald-400 px-3 py-1.5 rounded-sm border border-emerald-950/40">
                        Proposed Value: {formatCurrency(editingOrderStages.value)}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-[#111111] border border-[#222222] p-6 rounded-sm">
                    ...
                    <div className="space-y-3">
                      <h4 className="text-xs font-mono font-bold text-white uppercase tracking-widest">Apply Saved Stage Template</h4>
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          ...
                          {templates.map(t => (
                            <option key={t.id} value={t.id}>{t.title} ({t.domainName}) - {t.checklist?.length || 0} items</option>
                          ))}
                        </select>
                        <button
                          onClick={handleApplyTemplate}
                          disabled={!selectedTemplateId}
                          className="px-4 py-2 bg-white hover:bg-[#E5E5E5] text-black text-[10px] font-bold font-mono uppercase tracking-widest rounded-sm disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer transition-all"
                        >
                          Apply
                        </button>
                      </div>
                      ...
                      {templates.length > 0 && (
                        <div className="pt-2">
                          <span className="text-[9px] font-mono text-[#444444] uppercase tracking-wider block mb-1">Your saved protocols:</span>
                          <div className="flex flex-wrap gap-1.5">
                            {templates.map(t => (
                              <div key={t.id} className="flex items-center space-x-1 bg-[#1A1A1A] border border-[#222222] px-2 py-0.5 rounded-sm text-[10px] font-mono text-[#888888]">
                                <span className="truncate max-w-[120px]">{t.title}</span>
                                <button onClick={() => handleDeleteTemplate(t.id)} className="text-red-500 hover:text-red-400 font-bold ml-1">×</button>
                              </div>
                            ))}
                          </div>
                        </div>
                    ...
                    <div className="space-y-3 border-t md:border-t-0 md:border-l border-[#222222] pt-4 md:pt-0 md:pl-6">
                      <h4 className="text-xs font-mono font-bold text-white uppercase tracking-widest">Save Current Layout as Template</h4>
                      <p className="text-[11px] text-[#666666] leading-relaxed">
                        Export this customized list of service stages and checklist items so you can reuse them on future orders.
                      </p>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          disabled={!newTemplateName.trim() || !editingOrderStages.stages?.length}
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="lg:col-span-5 bg-[#111111] border border-[#222222] p-6 rounded-sm space-y-4 h-fit">
                      <h3 className="text-xs font-mono font-bold text-white uppercase tracking-widest flex items-center justify-between">
                        ...
                        <span className="text-[9px] text-[#666666] normal-case">Step-by-step dispatch configuration</span>
                      </h3>

                      <form onSubmit={handleAddStage} className="space-y-4 text-xs font-mono">
                        <div>
                          <label className="block text-[#666666] uppercase text-[9px] tracking-wider mb-1">Stage Action/Title *</label>
                          <input
                            type="text"
                            required
                        ...
                        <div>
                          <label className="block text-[#666666] uppercase text-[9px] tracking-wider mb-1">Operational Domain *</label>
                          <select
                            required
                            value={stageDomainId}
                        ...
                        <div>
                          <label className="block text-[#666666] uppercase text-[9px] tracking-wider mb-1">Assign Technician (With Free/Busy Checks) *</label>
                          <select
                            value={stageWorkerId}
                        ...
                        <div className="pt-2 border-t border-[#222222] space-y-2">
                          <label className="block text-[#666666] uppercase text-[9px] tracking-wider">Protocol Verification Checklist</label>
                          <p className="text-[10px] text-[#555555] leading-normal mb-2">
                            Add compliance items the technician must verify/complete before submitting.
                          </p>
                          ...
                          {checklistItems.length > 0 && (
                            <div className="space-y-1.5 pt-2 max-h-[150px] overflow-y-auto">
                              {checklistItems.map((item, idx) => (
                                <div key={idx} className="flex items-center justify-between bg-[#0A0A0A] p-2 border border-[#222222] rounded-sm text-xs">
                                  <span className="text-slate-400 truncate max-w-[200px]">{idx + 1}. {item}</span>
                                  <button
                                    type="button"
                        ...
                        <div className="pt-4 border-t border-[#222222]">
                          <button
                            type="submit"
                            className="w-full py-2.5 bg-white hover:bg-[#E5E5E5] text-black text-xs font-bold font-mono uppercase tracking-widest rounded-sm cursor-pointer transition-colors"
                          >
                            Add Stage to Order
                          </button>
                        </div>
                    ...
                    <div className="lg:col-span-7 bg-[#111111] border border-[#222222] p-6 rounded-sm space-y-4">
                      <h3 className="text-xs font-mono font-bold text-white uppercase tracking-widest">
                        Configured Service Delivery Stages ({editingOrderStages.stages?.length || 0})
                      </h3>

                      {(!editingOrderStages.stages || editingOrderStages.stages.length === 0) ? (
                        <div className="p-12 text-center text-[#555555] text-xs font-mono border border-dashed border-[#222222] rounded-sm">
                          No service execution stages have been defined for this field order yet. Use the manual form or select a saved template above.
                        </div>
                      ) : (
                        <div className="space-y-4">
                          {editingOrderStages.stages.map((stg, sIdx) => (
                            <div key={stg.id} className="p-4 bg-[#0A0A0A] border border-[#222222] rounded-sm space-y-3 relative group">
                              ...
                              <div>
                                ...
                                <h4 className="text-sm font-semibold text-white mt-1.5 pr-20">{stg.title}</h4>
                              </div>

                              <div className="grid grid-cols-2 gap-4 text-xs font-mono py-2 border-y border-[#1A1A1A] text-[#888888]">
                                <div>
                                  <span className="text-[9px] text-[#444444] uppercase block mb-1">Assigned Specialist</span>
                                  {stg.status === 'Completed' ? (
                                    <span className="text-white font-bold">{stg.assignedWorkerName || 'Unassigned'}</span>
                                  ) : (
                                    <select
                                      value={stg.assignedWorkerId || ''}
                                      onChange={(e) => {
                                        const wId = e.target.value;
                                        dbStore.updateOrderStageDetails(editingOrderStages.id, stg.id, {
                                          assignedWorkerId: wId || undefined,
                                          assignedWorkerName: wId ? (teamMembers.find(t => t.id === wId)?.name || 'Unassigned') : undefined
                                        });
                                      }}
                                ...
                                <div>
                                  <span className="text-[9px] text-[#444444] uppercase block">Stage Progress</span>
                              ...
                              <div className="space-y-1.5">
                                <span className="text-[9px] font-mono text-[#555555] uppercase tracking-wider block">Compliance Checklist:</span>
                                {(!stg.checklist || stg.checklist.length === 0) ? (
                                  <span className="text-[10px] text-[#444444] italic font-mono">No checklist protocols assigned.</span>
                                ) : (
                                  <div className="space-y-1">
                                    {stg.checklist.map((item) => (
                                      <div key={item.id} className="flex items-center space-x-2 text-xs font-mono">
                                        ...
                                        <span className={item.completed ? 'line-through text-[#444444]' : 'text-[#888888]'}>
                                          {item.text}
                                        </span>
                                      </div>
                                    ))}
          ...
          {activeSubTab === 'domains' && (
            ...
            >
              <div className="p-6 border-b border-[#222222] bg-[#0A0A0A]">
                <h2 className="text-lg font-serif italic font-light text-white">Active Company Domains</h2>
                <p className="text-xs text-[#666666] mt-1">
                  Read-only index of authorized company operational scopes (Managed only by Owner).
                </p>
              </div>

              {domains.length === 0 ? (
                <div className="p-12 text-center text-[#666666] text-xs">No domains defined.</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <thead>
                        ...
                        <tr key={domain.id} className="hover:bg-[#0D0D0D] transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-sm text-[10px] font-mono uppercase tracking-widest ${
                              domain.status === 'Active' ? 'bg-[#0D2A1D] text-emerald-400 border border-emerald-950/40' : 'bg-[#1A1A1A] text-[#666666] border border-[#222222]'
                            }`}>
                              {domain.status}
                            </span>
                          </td>
          ...
          {activeSubTab === 'team' && (
            ...
            >
              <div className="p-6 border-b border-[#222222] bg-[#0A0A0A]">
                <h2 className="text-lg font-serif italic font-light text-white">Authorized Team Roster</h2>
                <p className="text-xs text-[#666666] mt-1">
                  Read-only personnel list. Removal and edit privileges belong exclusively to company Owners.
                </p>
              </div>

              {teamMembers.length === 0 ? (
                <div className="p-8 text-center text-[#666666] text-xs">No other team members registered yet.</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <thead>
                    ...
                    <tbody className="divide-y divide-[#1A1A1A] text-xs">
                      {teamMembers.map((member) => (
                        <tr key={member.id} className="hover:bg-[#0D0D0D] transition-colors">
                          <td className="px-6 py-4 font-semibold text-white">{member.name}</td>
                          <td className="px-6 py-4 font-mono text-[#666666]">{member.email}</td>
                          <td className="px-6 py-4">
                            <span className="bg-[#1A1A1A] text-[#888888] px-2.5 py-1 rounded-sm text-[10px] font-mono font-bold uppercase tracking-widest border border-[#222222]">
                              {member.role}
                            </span>
