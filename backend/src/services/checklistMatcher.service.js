...
// Below this score, plain fuzzy matching isn't trusted — fallback to LLM.
...
function fuzzyMatch(transcript, checklistItems) {
  ...
  if (!labels.length) return { item: null, confidence: 0 };
...
/**
 * Fallback matching via Groq's LLM with negation detection
 */
async function llmMatch(transcript, checklistItems) {
  ...
  const itemList = checklistItems
    .map((i) => `- id: "${i._id || i.id}" | label: "${i.label}" | current status: "${i.status}"`)
    .join("\n");
...
- Do not rely only on which words appear — read the full meaning of the sentence.
...
{"matches": [{"id": "string", "new_status": "Completed" | "In Progress", "confidence": 0.0}]}`;
  ...
  const validIds = new Set(checklistItems.map((i) => String(i._id || i.id)));
  const safeMatches = (parsed.matches || []).filter((m) => validIds.has(String(m.id)));

  return safeMatches.map((m) => ({
    item: checklistItems.find((i) => String(i._id || i.id) === String(m.id)),
...
/**
 * Hybrid matcher: fuzzy first, LLM fallback when unsure.
 */
async function matchTranscriptToChecklist(transcript, checklistItems) {
  ...
  if (fuzzyResult.confidence >= FUZZY_CONFIDENCE_THRESHOLD && fuzzyResult.item) {
    return {
      matches: [
        {
